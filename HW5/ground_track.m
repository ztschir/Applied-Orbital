...
dec = [];
%r_rel = {};
J0 = 2453647.500000; %oct 4th, 2005
UT = 15.9222222; %15:55:20
T0 = (J0 - 2451545)/36525;
theta01 = 100.4606184 + 36000.77004*T0 + 0.000387933*T0^2 - 2.583*(10^-8)*T0^3;
theta0 = theta01 - 360*(floor(theta01/360));
thetaint = theta0 + 360.98564724*UT/24;
...
QxX = (R3*R2*R1)';
R = QxX*r;
theta = thetaint + we*(t - to);
Q = [ cos(theta) sin(theta) 0
-sin(theta) cos(theta) 0
0 0 1];
r_rel{i} = Q*R;
[alpha delta] = ra_and_dec_from_r(r_rel{i});
ra = [ra; alpha];
dec = [dec; delta];
end
end %find_ra_and_dec
...
for i = 1:4:size(ra)
    altitude(i) = norm(r_rel{i}) - Re;
    [hours minutes seconds] = time(57320 + i*4);
    fprintf('time = %g H : %g M : %g S UTC  Longitude = %g, Latitude = %g, altitude = %g , elevation angle = %g\n',hours, minutes, seconds,ra(i), dec(i), altitude(i), elevation(dec(i), ra(i), -38.325, 52.808))
        %,ra(i), dec(i), altitude(i), 0)%, elevation(dec(i), ra(i), altitude(i), -38.325, 52.808, .120383));
end
axis ([0 360 -90 90])
text( 52.808,-38.325, 'o Ground Station' ); 
text( ra(1), dec(1), 'o Start')
text(ra(end), dec(end), 'o Finish')
line([min(ra) max(ra)],[0 0], 'Color','k') %the equator
end %plot_ground_track

function [elv] = elevation (lat1, long1, lat2, long2)
    elv =atand((cosd(long1 - long2)*cosd(lat2) - .1512)/sqrt(1-cosd(long1 - long2)^2 * cosd(lat2)^2));
end

function [hours minutes seconds] = time(sec)
    days = floor(sec / 86400);
    hours = floor((sec / 3600) - (days * 24));
    minutes = floor((sec / 60) - (days * 1440) - (hours * 60));
    seconds = floor(mod(sec, 60));
end
% ~~~~~~~~~~~~~~~~~~~~~~~~~~
